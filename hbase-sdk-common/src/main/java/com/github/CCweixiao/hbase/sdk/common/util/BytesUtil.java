...
public final class BytesUtil {
     ...
     */
    public BytesUtil(final BytesUtil ibw) {
     ...
     */
    public BytesUtil(final byte[] bytes, final int offset, final int length) {
        this.bytes = bytes;
        this.offset = offset;
        this.length = length;
    }
    ...
    @Override
    public int hashCode() {
        return BytesUtil.hashCode(bytes, offset, length);
    }
    ...
    public String toString() {
        return BytesUtil.toString(bytes, offset, length);
    }
    ...
    static class ConverterHolder {
        ...
        protected static final class UnsafeConverter extends Converter {
            ...
            static {
                if (UNSAFE_UNALIGNED) {
                    theUnsafe = UnsafeAccess.THE_UNSAFE;
                } else {
    ...
    public static byte[][] toByteArrays(final String[] t) {
        ...
        for (int i = 0; i < t.length; i++) {
            result[i] = BytesUtil.toBytes(t[i]);
        }
        return result;
    }
    ...
    public static byte[][] toBinaryByteArrays(final String[] t) {
        byte[][] result = new byte[t.length][];
        for (int i = 0; i < t.length; i++) {
            result[i] = BytesUtil.toBytesBinary(t[i]);
        }
        return result;
    }
    ...
    public static boolean equals(List<byte[]> a, List<byte[]> b) {
        ...
        for (int i = 0; i < a.size(); ++i) {
            if (!BytesUtil.equals(a.get(i), b.get(i))) {
                return false;
            }
        }
        return true;
