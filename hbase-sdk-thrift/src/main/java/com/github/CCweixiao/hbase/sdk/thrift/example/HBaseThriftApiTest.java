...
public class HBaseThriftApiTest {
    ...
    public static void test1(){
        ...
        try {
            socket.open();
            allTableNames = hbaseClient.getTableNames().stream().map(ByteBufferUtil::byteBufferToString).collect(Collectors.toList());
    ...
    public static void test2(){
        ...
        while (true){
            ...
            try {
                socket.open();
                List<String> allTableNames = hbaseClient.getTableNames().stream().map(ByteBufferUtil::byteBufferToString).collect(Collectors.toList());
                System.out.println(allTableNames);
